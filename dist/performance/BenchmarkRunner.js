...
export class BenchmarkRunner {
    ...
    async benchmarkModelLoadTime() {
        ...
        await this.benchmark.run('Model Load Time', 'Time to initialize embedding model from cold start', async () => {
    ...
    async benchmarkInsertionThroughput() {
        ...
        let counter = 0;
        await this.benchmark.runThroughput('Single Insert Throughput', 'Number of single document insertions per second', async () => {
            await db.insert({
        ...
        counter = 0;
        await this.benchmark.runThroughput('Batch Insert Throughput (100 docs)', 'Number of batch insertions (100 docs each) per second', async () => {
     ...
     */
    async benchmarkSearchLatency(datasetSize) {
        ...
        let queryIndex = 0;
        await this.benchmark.run(`Search Latency (${datasetSize} vectors)`, `Average search time for k=10 on ${datasetSize} vector dataset`, async () => {
        ...
        queryIndex = 0;
        await this.benchmark.run(`Search with Filter (${datasetSize} vectors)`, `Search time with metadata filter on ${datasetSize} vectors`, async () => {
            const query = queries[queryIndex % queries.length];
            queryIndex++;
            await db.search({
                text: query,
    ...
    async benchmarkBatchOperations() {
        ...
        for (const batchSize of batchSizes) {
            await this.benchmark.run(`Batch Insert (${batchSize} docs)`, `Time to insert ${batchSize} documents in a single batch`, async () => {
    ...
    async benchmarkMemoryUsage() {
        ...
        await this.benchmark.profileMemory('Memory Usage During Insertion', 'Memory consumption while inserting 5000 documents', async () => {
            const batchSize = 100;
        ...
        await this.benchmark.profileMemory('Memory Usage During Search', 'Memory consumption during 100 search operations', async () => {
            for (let i = 0; i < 100; i++) {
    ...
    async benchmarkCachePerformance() {
        ...
        // Insert test data
        await db.insertBatch(Array.from({ length: 1000 }, (_, i) => ({
            text: this.generateTestDocument(i),
            metadata: { index: i },
        })));
        const query = 'machine learning';
        // First search (cold cache)
        await this.benchmark.run('Search (Cold Cache)', 'Search time with empty embedding cache', async () => {
            db.clearCaches();
            await db.search({ text: query, k: 10 });
        }, { iterations: 10, warmup: 0 });
        // Subsequent searches (warm cache)
        await this.benchmark.run('Search (Warm Cache)', 'Search time with cached embeddings', async () => {
            await db.search({ text: query, k: 10 });
