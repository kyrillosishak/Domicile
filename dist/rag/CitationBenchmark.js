 ...
 */
export const DEFAULT_LEGAL_CORPUS = [
...
export const DEFAULT_LEGAL_QUESTIONS = [
...
class LexOverlapReranker {
    ...
    async rerank(query, candidates) {
        ...
        const scored = candidates.map((c) => {
            const cText = c.metadata?.content ?? '';
            let overlap = 0;
            for (const t of tokenize(cText))
                if (qTerms.has(t))
                    overlap++;
 ...
 */
export async function benchmarkCitationAccuracy(options = {}) {
    const k = options.k ?? 3;
    ...
    const log = (m) => options.onProgress?.(m);
    ...
    for (const p of corpus)
        bm25.add(p.id, p.text);
    const variants = ['dense', 'dense+hybrid', 'dense+rerank', 'dense+hybrid+rerank'];
    ...
    for (const variant of variants) {
        ...
        for (const q of questions) {
            ...
            if (useHybrid && bm25.size() > 0) {
                ...
                const fused = reciprocalRankFusion(dense.map((r) => ({ id: r.id })), sparse.map((r) => ({ id: r.id })));
...
//# sourceMappingURL=CitationBenchmark.js.map