...
function makeVectors(count, dims, seed) {
...
function bruteForce(vectors, query, k, live) {
    const scored = [];
...
async function measureIndex(impl, vectors, opts, scale, log) {
    ...
    for (let i = 0; i < queries; i++)
        queryVecs.push(vectors[Math.floor(rng() * vectors.length)].slice());
    const latencies = [];
    ...
    for (let q = 0; q < queries; q++) {
        ...
        const hits = await impl.search(queryVecs[q], k);
        latencies.push(performance.now() - t0);
        for (const h of hits) {
            if (truth.has(Number(h.id.slice(1))))
                recallHits++;
            scoreMean += h.score;
        ...
        if (q === 0)
            firstScores = hits.map((h) => h.score);
    }
 ...
 */
export async function benchmarkIndex(scale, options = {}) {
    const opts = {
 ...
 */
export async function benchmarkSuite(scales = [
...
//# sourceMappingURL=IndexBenchmark.js.map