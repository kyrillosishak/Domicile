...
const TEMPLATES = {
    legal: {
        ...
        systemPrompt: 'You are a legal-research assistant operating under attorney-client privilege. ' +
    ...
    health: {
        ...
        systemPrompt: 'You are a clinical-record assistant. Use only the provided patient context. ' +
    ...
    blank: {
        ...
        dimensions: 384,
        llmHint: 'any',
        systemPrompt: 'You are a helpful assistant. Answer from the provided context and cite by [n].',
...
Options:
  ...
  --help                             Show this help`);
    ...
    const templateName = values.template ?? 'blank';
    const template = TEMPLATES[templateName];
    if (!template) {
        console.error(`Unknown template: ${templateName}. Choose legal|health|blank.`);
        return 1;
    }
    const outDir = resolve(values.out ?? './domicile-app');
    ...
    writeFileSync(resolve(outDir, 'package.snippet.json'), JSON.stringify({
        dependencies: {
    ...
    if (template.matterScope)
        console.log(`  matter scope       ${template.matterScope}`);
