...
export class TombstoneLog {
  ...
  private bitsCount = 0; // count of 1-bits roughly tracked
  private capacity = 0; // current label capacity

  ensureCapacity(label: number): void {
   ...
   */
  collectLive(allLabels: Iterable<number>, out: number[]): void {
    for (const label of allLabels) {
      if (!this.get(label)) out.push(label);
    }
  }
  ...
  load(bytes: Uint8Array): void {
    if (bytes.length < 8) throw new Error('TombstoneLog: header too short');
