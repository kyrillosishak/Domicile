...
import type { Index, IndexHit, IndexStats, SerializedIndex } from '../core/contracts';
import type { VectorRecord, Filter } from '../storage/types';
...
interface Node {
  id: string;
  vector: Float32Array;
  /** Connections per layer: layer → array of node ids. */
  links: Map<number, string[]>;
  /** Top layer this node occupies. */
  level: number;
  deleted: boolean;
}

export class HnswIndex implements Index {
  private config: Required<HnswIndexConfig>;
  ...
  async search(query: Float32Array, k: number, filter?: Filter): Promise<IndexHit[]> {
    this.ensureInitialized();
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (filter) {
        const filtered: IndexHit[] = [];
        for (const { id, dist } of candidates) {
          const node = this.nodes.get(id);
          if (!node || node.deleted) continue;
          filtered.push({ id, score: this.distanceToScore(dist) });
          if (filtered.length >= ef) break;
        }
  ...
  async serialize(): Promise<SerializedIndex> {
    this.ensureInitialized();
  ...
  async deserialize(serialized: SerializedIndex): Promise<void> {
    ...
    try {
      ...
      for (const n of parsed.nodes) {
        const node: Node = {
          ...
          links: new Map(n.links as [number, string[]][]),
          level: n.level,
          deleted: false,
        };
  ...
  private insertNode(id: string, vector: Float32Array): void {
    ...
    const node: Node = { id, vector, links: new Map(), level, deleted: false };
    for (let l = 0; l <= level; l++) node.links.set(l, []);
  ...
  private pruneNeighbor(node: Node, layer: number, m: number): void {
    const conns = node.links.get(layer)!;
    if (conns.length <= m) return;
    const scored = conns
  ...
  private searchLayer(query: Float32Array, ef: number): Array<{ id: string; dist: number }> {
  ...
  private searchLayerFrom(
    query: Float32Array,
    entryPoints: Array<{ id: string; dist: number }>,
    layer: number,
    ef: number
  ): Array<{ id: string; dist: number }> {
  ...
  private distanceToScore(dist: number): number {
    switch (this.config.metric) {
      case 'cosine':
  ...
  private estimateMemory(): number {
    ...
    for (const node of this.nodes.values()) {
      ...
      for (const conns of node.links.values()) bytes += conns.length * 16;
      bytes += 64;
    }
    return bytes;
  }
