...
export type LLMProviderKind = 'webllm' | 'wllama';
 ...
 */
const EMBEDDING_CATALOG: EmbeddingModelEntry[] = [
...
const LLM_CATALOG: LLMModelEntry[] = [
...
const TIER_RANK: Record<DeviceTier, number> = { low: 0, mid: 1, high: 2 };
...
export class ModelRegistry {
  private embedding = new Map<string, EmbeddingModelEntry>();
  private llm = new Map<string, LLMModelEntry>();

  constructor() {
    for (const e of EMBEDDING_CATALOG) this.embedding.set(e.id, e);
    for (const e of LLM_CATALOG) this.llm.set(e.id, e);
  }
  ...
  validateDimensions(embeddingModelId: string, indexDimensions: number): void {
    ...
    if (!entry) return; // unknown — best-effort, let the adapter report its own dims
  ...
  canRunEmbeddingModel(id: string, caps: Capabilities): CanRunResult {
    ...
    if (tierFail) return { ...tierFail, entry };
  ...
  canRunLLMModel(id: string, caps: Capabilities): CanRunResult {
    ...
    if (memFail) return { ...memFail, entry };
    return { canRun: true, reason: '', entry };
  }
  ...
  private checkTier(id: string, minTier: DeviceTier, caps: Capabilities): CanRunResult | null {
  ...
  private checkMemory(id: string, sizeGB: number, caps: Capabilities): CanRunResult | null {
    ...
    if (caps.deviceMemoryGB === undefined) return null;
