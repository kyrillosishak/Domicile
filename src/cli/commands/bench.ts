...
function parseSizes(raw: string | undefined): Array<{ size: number; dimensions: number }> {
  const defaultSizes = [
...
export async function cmdBench(args: string[]): Promise<number> {
...
Options:
  ...
  const sizes = (values.sizes as string | undefined)
    ? parseSizes(values.sizes as string).map((s) => ({ ...s, dimensions: dims }))
 ...
 */
async function runCitationBenchmark(k: number): Promise<number> {
  ...
  for (const v of result.variants) {
    console.log(
      `  ${v.variant.padEnd(22)} ${v.citationRecallAtK.toFixed(3).padStart(6)}   ${v.meanExpectedRank.toFixed(2).padStart(7)}`
    );
  }
