...
export function looksLikeNdjson(raw: string): boolean {
  ...
  for (const line of raw.split('\n')) {
    const t = line.trim();
    if (!t) continue;
    if (!t.startsWith('{') || !t.endsWith('}')) return false; // pretty-printed object spans lines
    lines++;
...
export function parseNdjson(raw: string): ExportData {
  ...
  for (const line of raw.split('\n')) {
    const t = line.trim();
    if (!t) continue;
    const rec = JSON.parse(t) as { type: string; data: any };
    switch (rec.type) {
      case 'metadata': {
        const m = rec.data;
...
Options:
  ...
  const input = resolve((values.in as string) ?? './export.json');
  ...
  const data: ExportData = looksLikeNdjson(raw)
    ? parseNdjson(raw)
